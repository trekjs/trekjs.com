...
module.exports = function fetchDocs (settings, callback) {
  ...
  var url = 'https://api.github.com/repos/trekjs/trek/tarball/' + settings.version
  settings.tmpDir = path.join(os.tmpDir(), 'trek-tmp-download')

  // Create temp directory to fetch tarball into
  mkdir(settings.tmpDir, function (error) {
    if (error) return callback(error)
    var token = process.env.TREK_ACCESS_TOKEN
    var filename = 'trek.tar.gz'
    var opts = {
      headers: {
        'user-agent': 'Trek',
...
function constructSourceUrl (path) {
  var baseUrl = 'https://github.com/trekjs/trek/blob/master/'
